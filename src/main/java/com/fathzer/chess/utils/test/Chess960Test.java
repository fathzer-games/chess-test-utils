...
public class Chess960Test<B extends IBoard<M>, M> extends AbstractAdaptableTest<B, M> {
	...
	void rookDoesntMoveCastling() {
		final B board = u.fenToBoard("nrk1brnb/pp1ppppp/1q6/2p5/3P4/1NBQ1N2/1PP1PPPP/1RK2R1B w KQkq - 2 10", CHESS960);
		final M move = u.move(board, "c1f1");
		assertTrue(board.isLegal(move));
		board.makeMove(move);
	...
	void castlingOnStandardStartPosition() {
		...
		final B board = u.fenToBoard("rnbqk2r/pppp1ppp/3b1n2/4p3/4P3/3B1N2/PPPP1PPP/RNBQK2R w KQkq - 0 1", CHESS960);
		final M move = u.move(board, "e1h1");	
		assertTrue(board.isLegal(move));
	}
